...
class Curl
{
    ...
    public $islog = false;
    public $logfilePath = '/Users/xiaogangchen/www/nuonuo/data/runtime/Logs/curlLog.txt';
    ...
    private $_header, $_headerMap, $_errorno, $_error, $_status, $_info;
    ...
    public function exec($url, $options, $debug = false)
    {
        ...
        //记录日志1（调试可能发生的异常）
        $this->islog && error_log("\r\n执行前：". json_encode(['time'=>date('Y-m-d H:i:s'),'url'=>$url,'options'=>$options])."\r\n",3,$this->logfilePath);

        $output = curl_exec($ch);
        $this->islog && error_log("执行中：curl_exec\r\n",3,$this->logfilePath);

        $this->_status = curl_getinfo($ch, CURLINFO_HTTP_CODE);

        //if(!$output || ($this->_error = trim(curl_error($ch)))){
        if(!$output){
        ...
        //记录日志2（调试可能发生的异常）
        $this->islog && error_log("执行后：".json_encode(['time'=>date('Y-m-d H:i:s'),'errorno'=>$this->_errorno,'error'=>$this->_error,'info'=>$this->_info])."\r\n",3,$this->logfilePath);
