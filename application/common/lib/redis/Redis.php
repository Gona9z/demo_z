...
class Redis
{
    ...
    {
        empty($config) && $config = C('REDIS_CONNECTION');//如果没定义就直接使用TP自定义的配置
        ...
        if(isset($config['AUTH']) && !is_null($config['AUTH'])) {
            $this->auth( $config['AUTH'] );
            $this->auth = $config['AUTH'];
        }

        if(isset($config['DB_INDEX']) && !is_null($config['DB_INDEX'])){
